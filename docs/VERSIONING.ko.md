...
- `MINOR`: 호환 가능한 feature 추가, UI flow, CLI command, storage addition, wiki capability.
...
`1.0.0` 전에는 public CLI behavior, storage format, UI flow가 바뀔 수 있습니다. 그래도 변경 사항은 명확히 문서화해야 합니다.
...
- `/prompts`, `/settings`, `/ingest file`, `/query`, `/lint wiki`, `/log recent`, `/sources`, `/wiki list`, `/exit`가 CLI chat에서 동작함.
...
Legacy GUI smoke는 수동입니다.

- GUI가 시작되고 3패널 layout을 표시함.
- Command proposal과 run log가 Activity panel에 렌더링됨.
